    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter.apply {
            ...
            listener = object : PlaylistAdapter.PlayItemSelectListener {
                override fun onPlayItemSelected(m3UItem: M3UItem, channelIndex: Int) {
                    m3UItem.itemUrl?.let { url ->
                        val intent = Intent(this@ListActivity, PlayerExo::class.java)
                        intent.putExtra("Name", m3UItem.itemName)
                        intent.putExtra("Url", url)
                        intent.putExtra("selectedCategoryIndex", selectedCategoryIndex)
                        intent.putExtra("selectedChannelIndex", channelIndex)
